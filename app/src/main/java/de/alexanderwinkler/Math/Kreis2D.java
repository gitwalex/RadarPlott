...
public class Kreis2D implements Serializable {
	...
	public final boolean isAufKreis(Punkt2D p) {
		double s = Math
				.sqrt(((p.getX() - mittelpunkt.getX()) * (p.getX() - mittelpunkt
						.getX()))
						+ ((p.getY() - mittelpunkt.getY()) * (p.getY() - mittelpunkt
								.getY())));
		double d = s - radius;
	 ...
	 * @param p
	 *            Punkt
	 * @return true: Punkt liegt innerhalb des Kreises, false: Punkt liegt auf
	 *         oder ausserhalb des Kreises
	 */
