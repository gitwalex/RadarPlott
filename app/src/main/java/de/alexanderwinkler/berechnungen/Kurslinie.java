...
public class Kurslinie extends Gerade2D implements Serializable {
	...
	public Punkt2D getPunktinFahrtrichtungnachDauer(Punkt2D p, double minuten) {
		if (!isPunktAufKurslinie(p)) {
			Log.d(LOGTAG,
					"Fehler bei Kurslinie.getPunktinFahrtrichtungnachDauer(Punkt p, double minuten)"
							+ ", Parameter: " + p.toString() + " / " + minuten);
			throw new IllegalArgumentException(
					"Punkt liegt nicht auf Kurslinie");
		}
		double x = p.getX();
		double y = p.getY();
		x = x + richtungsvektor.getEndpunkt().getX() * geschwindigkeit
				* minuten / 60d;
		y = y + richtungsvektor.getEndpunkt().getY() * geschwindigkeit
				* minuten / 60d;
		return new Punkt2D(x, y);
	}
	...
	public Punkt2D getPunktinFahrtrichtungmitAbstand(double abstand) {
		...
		if (s[0] == null & s[1] == null) {
			Log.d(LOGTAG,
					"Fehler bei Kurslinie.getPunktinFahrtrichtungmitAbstand(double abstand), Parameter: "
							+ abstand);
			throw new IllegalArgumentException(
