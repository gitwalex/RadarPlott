...
/**
 * Erweitert die Lage. Bietet Funktionen, um die Veraenderung zur urspruenglichen Lagedarzustellen.
 *
...
public class Manoever extends Lage {
     ...
     */
    public Manoever(Lage lage, float neuerkurs, float manoeverminuten, float fahrt) {
        ...
        eEigFahrt = fahrt;
        pos1_a = new Punkt2D(
                (float) (-(fahrt / (60 / eIntervall) * Math.sin(Math.toRadians(neuerkurs)))),
                (float) (-(fahrt / (60 / eIntervall) * Math.cos(Math.toRadians(neuerkurs)))));
        // wahre Position a bei zweiter Peilung
        pos2_a = p;
        // Position b zum Zeitpunkt Manoeverminuten
        Punkt2D manoeverOrt =
                lage.b2.getPunktinFahrtrichtungnachDauer(lage.b2.getAktPosition(), manoeverminuten);
