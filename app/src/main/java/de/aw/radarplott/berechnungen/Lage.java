...
public class Lage implements Konstanten, Serializable {
    ...
    protected float eEigKurs, eEigFahrt, eIntervall, rasp1, eDistanz1, rasp2,
            eDistanz2;
    ...
    public Lage(boolean northup, float intervall, Punkt2D... punkt) {
        ...
        eDistanz1 = p.getDistance(pos1_b);
        eDistanz2 = p.getDistance(pos2_b);
    }
     ...
     *         true: Peilung erfolgt mit rechtweisend Nord
     * @param werte
     *         float-Array mit folgenden Inhalt (Reihenfolge beachten): - Kurs A (rechtweisend) -
     ...
     */
    public float getCPAEntfernung() {
        Punkt2D cpa = a.getCPAOrt(b2);
        float d = b2.getEntfernung(cpa);
        return d;
    }
     ...
     */
    public float getEntfernungKurslinie() {
        Punkt2D p = a.getSchnittpunkt(b2);
        float d = b2.getEntfernung(p);
        return d;
    }
    ...
    public float getRASP2() {
        float angle = new Vektor2D(new Punkt2D(0, 0), b2.aktuellerStandort)
                .getWinkelRechtweisendNord();
        angle = 360f - a.getWinkelRW() + angle;
    ...
    private final void init() {
        ...
        pos1_a = new Punkt2D((float) -(eEigFahrt / (60 / eIntervall) * Math
                .sin(Math.toRadians(eEigKurs))), (float)
        ...
        pos0_b = pos1_a.getPunkt(eDistanz1, rasp1);
        // wahre Position b bei erster Peilung
        pos1_b = pos2_a.getPunkt(eDistanz1, rasp1);
        // Position b bei zweiter Peilung
        pos2_b = pos2_a.getPunkt(eDistanz2, rasp2);
    ...
    public float newCPA(float newcpa) throws IllegalArgumentException {
        ...
        Punkt2D aktA = a.aktuellerStandort;
        Punkt2D aktB = b2.aktuellerStandort;
        ...
        float abstandA = Math.abs(mittelpunkt.getDistance(aktA));
        // Kreis aufspannen
        ...
        if (erg == null) {
            Log.d(LOGTAG,
                    "Fehler bei Lage.newCPA(float newcpa), Parameter: " + newcpa);
        ...
        Vektor2D v = new Vektor2D(b2.getAktuellerStandort(), erg[0]);
        ...
        v = new Vektor2D(b2.getAktuellerStandort(), erg[1]);
    ...
    public void setFahrtaenderung(float neueGeschwindigkeit) {
